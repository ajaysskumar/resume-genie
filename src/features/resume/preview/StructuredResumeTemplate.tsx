  ...
  return (
    <div className="space-y-5 text-[11pt] leading-[1.32] text-slate-800">
      <header className="border-b border-slate-300 pb-4 text-center">
        <h1 className="text-[29px] font-bold leading-none tracking-tight text-slate-950"><FormattedText>{personal.fullName || 'Your Name'}</FormattedText></h1>
        <p className="mt-1 text-[15px] font-semibold leading-tight text-slate-800"><FormattedText>{personal.headline || 'Your Headline'}</FormattedText></p>
      ...
      {summary && (
        <section>
          ...
          <p className="px-3.5 text-justify text-[11px] leading-[1.42] whitespace-pre-wrap"><FormattedText>{summary}</FormattedText></p>
        </section>
      )}
          ...
          <div className="space-y-3 px-3.5">
            {education.map((item) => (
              <article key={item.id}>
                <div className="flex items-start justify-between gap-4">
                  <div>
                    <h3 className="text-[11px] font-bold text-slate-900"><FormattedText>{item.degree}</FormattedText></h3>
                    <p className="text-[11px]"><FormattedText>{item.institution}</FormattedText>{item.location ? `, ${item.location}` : ''}</p>
                  </div>
                ...
                {item.grade && <p className="text-[10px]"><FormattedText>{item.grade}</FormattedText></p>}
              </article>
            ))}
          </div>
        </section>
          ...
          <div className="space-y-1.5 px-3.5 text-[11px] leading-[1.35] text-slate-700">
            {skillGroups.map((group) => (
              <div key={group.category} className="grid grid-cols-[minmax(6rem,0.8fr)_minmax(0,3fr)] items-baseline gap-x-4">
                <strong className="text-slate-950"><FormattedText>{group.category}</FormattedText></strong>
                <span><FormattedText>{group.skills.join(' · ')}</FormattedText></span>
              </div>
            ))}
          </div>
        </section>
            ...
            {projects.map((project) => (
              <article key={project.id}>
                <div className="flex items-baseline justify-between gap-3">
                  <h3 className="text-[11px] font-bold text-slate-900"><FormattedText>{project.name}</FormattedText>{project.organization ? <>, <FormattedText>{project.organization}</FormattedText></> : null}</h3>
                ...
                <p className="text-[11px] leading-[1.38]"><FormattedText>{project.description}</FormattedText></p>
                {project.technologies.length > 0 && <p className="text-[10px] text-slate-600"><FormattedText>{project.technologies.join(' · ')}</FormattedText></p>}
              </article>
            ))}
          </div>
        </section>
          ...
          <div className="space-y-2 px-3.5">
            {certifications.map((certification) => (
              <article key={certification.id} className="flex items-baseline justify-between gap-4 text-[11px]">
                <div><strong><FormattedText>{certification.name}</FormattedText></strong><span> · <FormattedText>{certification.issuer}</FormattedText></span></div>
