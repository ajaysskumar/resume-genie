  ...
  return (
    <div className="text-slate-800">
      <header className="border-l-4 border-violet-500 pl-5">
        ...
        <h1 className="mt-2 text-3xl font-bold text-slate-950"><FormattedText>{personal.fullName || 'Your Name'}</FormattedText></h1>
            ...
            <div className="space-y-6">
              {experience.map((exp) => (
                <article key={exp.id} className="grid grid-cols-[92px_1fr] gap-4">
                  ...
                  <div><h3 className="text-sm font-bold text-slate-950"><FormattedText>{exp.position}</FormattedText></h3><p className="mt-1 text-sm font-semibold text-slate-600"><FormattedText>{exp.company}</FormattedText>{exp.location ? ` · ${exp.location}` : ''}</p><ul className="mt-2 space-y-1 text-sm leading-6 text-slate-600">{exp.bullets.map((bullet) => <li key={bullet.id}>• <FormattedText>{bullet.text}</FormattedText></li>)}</ul></div>
                </article>
              ))}
            </div>
          </section>
